...
import {
  Search, Copy, Check, Loader2, ChevronRight, Youtube, X,
...
export default function YouTubeAnalyzer() {
  ...
  // Auto-analyze connected channel on mount
  useEffect(() => {
            ...
            {!ytChannel ? (
              <motion.div key="not-connected" initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }}
                ...
                <div className="w-16 h-16 rounded-2xl flex items-center justify-center"
                  style={{ background: YT_CONTAINER }}>
                  <Youtube className="w-8 h-8 text-red-500" />
                </div>
                <div>
                  <p className="font-bold text-lg text-[#191c1d] dark:text-white mb-2">Connect Your Channel</p>
                  <p className="text-sm text-[#757684] max-w-xs">Connect your YouTube channel to get personalized analysis and content recommendations</p>
                </div>
              ...
              <motion.div key="connected" initial={{ opacity:0 }} animate={{ opacity:1 }} className="space-y-4">
                ...
                <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-[#e1e3e4] dark:border-gray-700">
                  <div className="flex items-center gap-3 mb-4">
                    {ytChannel.channel_thumbnail && (
                      <img src={ytChannel.channel_thumbnail} alt="" className="w-12 h-12 rounded-full shrink-0" />
                    )}
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      { icon: Users, label: 'Subscribers', val: formatNum(Number(ytChannel.subscribers || 0)), color: '#ff0000' },
                      { icon: Video, label: 'Videos', val: formatNum(Number(ytChannel.video_count || 0)), color: '#ff6b35' },
                      { icon: Eye, label: 'Total Views', val: formatNum(Number(ytChannel.total_views || 0)), color: '#ff9900' },
                    ].map((stat, i) => (
                ...
                {loading && (
                  <div className="flex flex-col items-center justify-center py-10 gap-3">
                    ...
                    <p className="text-sm text-[#757684]">Analyzing {ytChannel.channel_name}...</p>
                  </div>
                )}
        ...
        {activeTab === 'search' && (
          <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} className="space-y-4">
            ...
            <div className="relative">
              <div className="flex gap-2">
                <div className="relative flex-1">
                  ...
                  {channelUrl && (
                    <button onClick={handleClear} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#757684]">
                      <X className="w-4 h-4" />
                    </button>
                  )}
                </div>
              ...
              {showDropdown && (
                <div ref={dropdownRef}
                  className="absolute top-full left-0 mt-1 bg-white dark:bg-gray-800 border border-[#e1e3e4] dark:border-gray-700 rounded-2xl shadow-xl z-50 overflow-hidden"
                ...
                <div className="flex flex-wrap gap-2">
                  {POPULAR_CHANNELS.map(ch => (
                    <button key={ch} onClick={() => handleAnalyze(ch)}
                      className="px-3 py-1.5 rounded-full border border-[#e1e3e4] bg-white dark:bg-gray-800 text-xs text-[#454652] dark:text-gray-300 hover:border-red-300 hover:text-red-500 transition-colors">
                      {ch}
                    </button>
                  ))}
                </div>
            ...
            {loading && (
              <div className="flex flex-col items-center justify-center py-10 gap-3">
                <Loader2 className="w-8 h-8 animate-spin text-red-500" />
                <p className="text-sm text-[#757684]">Analyzing {channelUrl}...</p>
              </div>
            )}
            ...
            {result.content_pillars.map((pillar: string, i: number) => (
              <div key={i} className="flex items-center gap-2 px-3 py-2 rounded-xl" style={{ background: '#ffebee' }}>
                <div className="w-5 h-5 rounded-full flex items-center justify-center text-white text-[10px] font-bold shrink-0"
                  style={{ background: YT_GRAD }}>{i+1}</div>
              ...
              <div key={i} className="flex items-start gap-2 p-3 rounded-xl bg-[#f8f9fa] dark:bg-gray-700">
                ...
                <button onClick={() => onCopy(idea, `idea-${i}`)} className="shrink-0">
                  {copied === `idea-${i}`
                    ...
                    : <Copy className="w-3.5 h-3.5 text-[#757684]" />}
                </button>
              </div>
            ))}
          </div>
