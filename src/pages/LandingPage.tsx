            ...
            {step === 'status' && statusData && (
              <div className="sr-fade-in">
                ...
                <div style={{ marginBottom:14 }}>
                  <p style={{ fontSize:10,color:'rgba(255,255,255,0.3)',letterSpacing:'0.1em',textTransform:'uppercase',marginBottom:7,fontFamily:'Roboto,sans-serif' }}>Your referral code</p>
                  <div style={{ display:'flex',gap:8 }}>
                    <div style={{ flex:1,background:'rgba(255,255,255,0.03)',border:'1px solid rgba(139,92,246,0.2)',borderRadius:10,padding:'10px 14px',fontSize:14,fontWeight:700,letterSpacing:'0.08em',color:'#a78bfa',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap' }}>
                      {statusData.referral_code}
                    </div>
                    <button onClick={() => { navigator.clipboard.writeText(statusData.referral_code); setCodeCopied(true); setTimeout(()=>setCodeCopied(false),2000); }}
                      style={{ background:codeCopied?'rgba(74,222,128,0.1)':'rgba(139,92,246,0.1)',border:`1px solid ${codeCopied?'rgba(74,222,128,0.28)':'rgba(139,92,246,0.28)'}`,borderRadius:10,padding:'10px 14px',color:codeCopied?'#4ADE80':'#a78bfa',fontSize:12,cursor:'pointer',whiteSpace:'nowrap',fontFamily:'Roboto,sans-serif',transition:'all .2s' }}>
