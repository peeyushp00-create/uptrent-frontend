...
import { Search, X, Sparkles, Instagram, Youtube } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa] dark:bg-gray-900 relative overflow-hidden flex flex-col items-center justify-center px-5 py-12">
      ...
      <motion.div animate={{ scale: [1, 1.15, 1], opacity: [0.06, 0.12, 0.06] }}
        ...
        style={{ background: `radial-gradient(ellipse, ${isIG ? PRIMARY : '#ff0000'}40, transparent 65%)`, filter: "blur(80px)" }} />
      <motion.div animate={{ scale: [1.1, 1, 1.1], opacity: [0.04, 0.08, 0.04] }}
        ...
        style={{ background: `radial-gradient(circle, ${isIG ? PRIMARY : '#ff0000'}40, transparent 70%)`, filter: "blur(80px)" }} />
      ...
      <div className="w-full max-w-2xl relative z-10 flex flex-col items-center gap-8">
        ...
        {/* Connect Channel Button — syncs with sidebar toggle */}
          ...
          <div className="relative flex gap-2">
            <div className="relative flex-1">
              ...
              <input type="text" value={search} onChange={e => setSearch(e.target.value)}
                ...
                style={{ background: 'white', border: `2px solid ${search ? (isIG ? PRIMARY : '#ff0000') : '#e1e3e4'}`, boxShadow: search ? `0 0 0 4px ${isIG ? PRIMARY : '#ff0000'}15` : 'none' }} />
              {search && (
            ...
            <AnimatePresence mode="wait">
              {chips.map((chip, i) => (
                <motion.button key={`${platform}-${chip}`}
                  ...
                  style={search === chip
                    ? { background: activeGrad, color: '#fff', borderColor: 'transparent' }
