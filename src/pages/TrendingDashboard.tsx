...
import {
  ...
  Sparkles, Loader2, Search, X,
  Copy, Check, Globe, RefreshCw, Instagram, Youtube, Hash
} from "lucide-react";
...
// ── Static micro niche map for instant results ──
const MICRO_NICHES: Record<string, MicroNiche[]> = {
  ...
  gaming: [
    ...
    { name: "Best Gaming Setup Under 30K", hashtags: ["#gamingsetup","#budget","#pcgaming"], why: "Budget setups = massive aspiration content", virality: 92 },
    { name: "Free Fire Rank Push Guide", hashtags: ["#freefire","#rankpush","#gaming"], why: "Rank content = loyal gaming audience", virality: 88 },
    { name: "Gaming Career in India", hashtags: ["#esports","#gaingcareer","#india"], why: "Career content gets shares from parents", virality: 86 },
  ...
  motivation: [
    ...
    { name: "1% Better Every Day", hashtags: ["#atomic","#habits","#selfimprovement"], why: "James Clear trend = proven viral topic", virality: 93 },
    { name: "Rich vs Wealthy Mindset India", hashtags: ["#mindset","#wealth","#money"], why: "Mindset + money = double virality", virality: 95 },
  ],
  business: [
    { name: "How to Start Business with 0 Investment", hashtags: ["#business","#startup","#zeroinvestment"], why: "Zero investment hook = massive clicks", virality: 97 },
    ...
    { name: "Top Business Ideas Under 1 Lakh", hashtags: ["#businessideas","#startup","#india"], why: "Actionable budget business = shares", virality: 96 },
...
// Get micro niches for any topic
function getMicroNiches(query: string): MicroNiche[] {
  ...
  return [
    ...
    { name: `${query} in 2026 — What's Changed`, hashtags: [`#${q.replace(/\s/g,'')}`, "#2026", "#trending"], why: "Year-specific content ranks well", virality: 87 },
...
export default function TrendingDashboard() {
  ...
  const [googleUpdated, setGoogleUpdated] = useState("");
  ...
  const [activeTab, setActiveTab] = useState<"google" | "instagram" | "all">("instagram");
  const [searchQuery, setSearchQuery] = useState("");
  const [microNiches, setMicroNiches] = useState<MicroNiche[]>([]);
  const [showMicroNiches, setShowMicroNiches] = useState(false);
  ...
  const fetchGoogleTrends = async () => {
    ...
    try {
      const res = await fetch(`${BASE}/api/trends/google`);
      const data = await res.json();
      setGoogleTrends(data.trends || []);
      setGoogleUpdated(data.updated || "");
      setIsFallback(data.isFallback || false);
    } catch (e) { console.error(e); }
    finally { setGoogleLoading(false); }
  };

  useEffect(() => { fetchGoogleTrends(); }, []);

  // ✅ When user searches — show micro niches
  const handleSearch = (q: string) => {
    setSearchQuery(q);
    if (q.trim().length > 1) {
      setMicroLoading(true);
      setShowMicroNiches(true);
  ...
  const filteredTopics = searchQuery.trim()
    ...
    : activeTab === "instagram"
    ? topics.filter(t => t.type === "instagram" || !t.type)
    : topics;

  const tabs = [
    { id: "instagram", label: "📸 Instagram", count: topics.filter(t => t.type === "instagram" || !t.type).length },
    { id: "google", label: "🌐 Google", count: googleTrends.length },
      ...
      <div className="sticky top-0 z-10 bg-background/80 backdrop-blur-md border-b border-border px-4 py-3">
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            {!isFallback && googleTrends.length > 0 && (
              <span className="text-xs px-2 py-0.5 rounded-full flex items-center gap-1"
                style={{ background: `${IG}15`, color: IG }}>
                <span className="w-1.5 h-1.5 rounded-full inline-block animate-pulse" style={{ background: IG }}></span>
                Live
              </span>
            )}
          </div>
          <button onClick={fetchGoogleTrends} disabled={googleLoading}
      ...
      <div className="max-w-2xl mx-auto px-4 py-4 pb-24 space-y-4">
        ...
        <div className="relative">
          ...
          <input ref={inputRef} value={searchQuery}
            ...
            placeholder="Search niche to discover micro-niches (e.g. Fitness, Finance)..."
          ...
          {showMicroNiches && (
            <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
              ...
              <p className="text-xs text-muted-foreground">
                These specific sub-topics have high viral potential on {isIG ? "Instagram Reels" : "YouTube Shorts"}
              </p>

              {microLoading ? (
                <div className="flex items-center justify-center py-8 gap-2">
                  <Loader2 className="w-5 h-5 animate-spin" style={{ color: ac }} />
                  <p className="text-xs text-muted-foreground">Finding viral micro-niches...</p>
                </div>
              ) : (
                    ...
                    <motion.div key={i}
                      ...
                      <div className="flex items-start justify-between gap-3 mb-3">
                        ...
                        <div className="flex flex-col items-center shrink-0">
                          <div className="text-lg font-bold" style={{ color: niche.virality >= 90 ? ac : niche.virality >= 80 ? "#22c55e" : "#f59e0b" }}>
                            {niche.virality}
                          </div>
          ...
          <>
            <div className="flex gap-1 p-1 rounded-2xl bg-card border border-border">
              {tabs.map(tab => (
                <button key={tab.id} onClick={() => setActiveTab(tab.id as any)}
                  className="flex-1 flex items-center justify-center gap-1 py-2 px-2 rounded-xl text-xs font-medium transition-all"
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm font-semibold text-foreground">Trending on Instagram India</p>
                  </div>
                </div>
                {loading ? (
                        ...
                        <motion.div key={topic.id || i}
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-0.5">
                              ...
                              <span className={`text-xs px-1.5 py-0.5 rounded-full shrink-0 ${scoreLabel.color}`}>{scoreLabel.label}</span>
                            </div>
                            <div className="flex items-center gap-2">
            ...
            {activeTab === "google" && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Globe className="w-4 h-4" style={{ color: IG }} />
                    <p className="text-sm font-semibold text-foreground">Google Trends India</p>
                  </div>
                  {googleUpdated && (
                    <p className="text-xs text-muted-foreground">
                      {new Date(googleUpdated).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}
                    </p>
                  )}
                </div>
                {isFallback && (
                  <div className="p-3 rounded-xl text-xs text-muted-foreground"
                    ...
                    ℹ️ Showing curated trending topics. Live data updates every 30 mins.
                  </div>
                )}
                {googleLoading ? (
                  <div className="flex justify-center py-12">
                  ...
                  <div className="space-y-2">
                    {googleTrends.map((trend, i) => (
                      <motion.div key={i}
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-foreground truncate">{trend.name}</p>
                          <div className="flex items-center gap-2 mt-0.5">
                            {trend.searches && <span className="text-xs text-muted-foreground">{trend.searches}</span>}
                            <span className={`flex items-center gap-0.5 text-xs ${momentumColor(trend.momentum)}`}>
                              <MomentumIcon momentum={trend.momentum} />{trend.momentum}
                            </span>
                          </div>
                        </div>
                        <button onClick={() => handleGenerateScript(trend)}
                          className="flex items-center gap-1 px-3 py-1.5 rounded-xl text-white text-xs font-medium shrink-0"
                          style={{ background: ag }}>
                          <Sparkles className="w-3 h-3" /> Script
                        </button>
