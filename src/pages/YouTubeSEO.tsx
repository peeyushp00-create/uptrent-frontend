...
import { Tag, Sparkles, Copy, Check, Loader2, Search } from "lucide-react";
...
export default function YouTubeSEO() {
  ...
  useEffect(() => {
    if (topic.trim().length > 0) {
      const filtered = SEO_SUGGESTIONS.filter(s =>
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
            ...
            <div className="bg-card border border-border rounded-2xl p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <button onClick={() => copyText(result.description, 'desc')} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground">
            ...
            <div className="bg-card border border-border rounded-2xl p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={() => copyText(result.tags?.map((t: any) => t.tag || t).join(', '), 'tags')} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground">
                ...
                {result.tags?.map((tag: any, i: number) => {
                  ...
                  return (
                    <div key={i} className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl border text-xs ${getScoreBg(score || 50)}`}>
                      ...
                      {score && <span className={`font-bold text-xs px-1 py-0.5 rounded-md bg-background/50 ${getScoreColor(score)}`}>{score}</span>}
                    </div>
                  );
                })}
              </div>
