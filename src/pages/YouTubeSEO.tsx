  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch(`${BASE}/api/youtube/seo`, {
        ...
        body: JSON.stringify({ topic, niche: user?.user_metadata?.niche, language: user?.user_metadata?.language }),
      });
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-4 pb-24">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Get optimized titles, description and tags for any video topic</p>
        </div>
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
            ...
            <div className="bg-card border border-border rounded-2xl p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <button onClick={() => copyText(result.description, 'desc')} className="flex items-center gap-1 text-xs text-muted-foreground">
            ...
            <div className="bg-card border border-border rounded-2xl p-4">
              <div className="flex items-center justify-between mb-3">
                <p className="text-xs font-bold uppercase text-green-400">🏷️ Tags</p>
                <button onClick={() => copyText(result.tags?.join(', '), 'tags')} className="flex items-center gap-1 text-xs text-muted-foreground">
