        ...
        <div className="relative">
          <div className="flex gap-2">
            ...
            <button onClick={() => handleGenerate()} disabled={loading}
              ...
              {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : t('scripts.generate')}
            </button>
          </div>
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
            ...
            <div className="bg-card border border-border rounded-2xl p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <button onClick={() => copyText(result.tags?.map((t: any) => t.tag || t).join(', '), 'tags')}
                  className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground">
                  {copied === 'tags' ? <Check className="w-3.5 h-3.5 text-green-400" /> : <Copy className="w-3.5 h-3.5" />} {t('scripts.copy_all')}
                </button>
              </div>
