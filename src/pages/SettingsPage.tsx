...
export default function SettingsPage() {
  ...
  const sampleText = {
    ...
    hinglish: "Hey guys, welcome back! Aaj main aapke saath kuch bahut important tips share karna chahta hoon. Maine dekha hai ki bahut saare creators struggle karte hain. But actually it's not that difficult!",
    manglish: "Hello guys! Innu njaan ningalude koodey share cheyyaan pokunnathu chila important tips aanu. Kurachu creators content undakkaan struggle cheyyunnathu. But actually athu athra difficult alla!",
  };
      ...
      >
          ...
          <div className="flex flex-wrap gap-2">
            {NICHES.map((n) => (
              <button
                ...
                style={niches.includes(n) ? { background: "linear-gradient(135deg, #3B82F6, #1D4ED8)" } : {}}
              >
                {niches.includes(n) && '✓ '}{n}
              </button>
            ))}
          ...
          <div className="flex gap-3">
            {PLATFORMS.map((p) => (
              <button
                ...
                style={platform === p.value ? { background: "linear-gradient(135deg, #3B82F6, #1D4ED8)" } : {}}
              >
                {p.label}
              </button>
            ))}
          ...
          <div className="flex flex-col gap-2">
            {LANGUAGES.map((l) => (
              <button
                ...
                style={language === l.value ? { background: "linear-gradient(135deg, #3B82F6, #1D4ED8)" } : {}}
              >
                {l.label}
              </button>
            ))}
          ...
          <div className="flex flex-col gap-2">
            {STYLES.map((s) => (
              <button
                ...
                style={style === s.value ? { background: "linear-gradient(135deg, #3B82F6, #1D4ED8)" } : {}}
              >
                {s.label}
              </button>
            ))}
        ...
        <div className="bg-card border border-border rounded-xl p-6 space-y-4">
          ...
          <button
            ...
            style={!isRecording ? { background: "linear-gradient(135deg, #3B82F6, #1D4ED8)" } : {}}
          >
            {isRecording ? (
